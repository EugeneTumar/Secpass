...
from ..db.secpass import db_add_secpass, db_del_secpass_by_id, db_get_secpass_by_id, db_get_secpasses_by_user_id
...
async def del_secpass(id: int, session: str):
    try:
        user_id = await get_user_id_by_session(session)
        secpass = await db_get_secpass_by_id(id)
        if secpass is None:
            raise SecpassNotFound
        if not secpass.user_id is user_id:
            raise AccessDenied
        await db_del_secpass_by_id(id)
