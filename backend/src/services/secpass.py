...
from ..schemas.secpass import DecryptFormSchema, DecryptSecpassSchema, EncryptFormSchema, UpdateSecpassLabelSchema
...
async def update_secpass_label(updateData:UpdateSecpassLabelSchema, session: str):
    try:
        user_id = await get_user_id_by_session(session)
        secpass = await db_get_secpass_by_id(updateData.secpass_id)
        ...
        await db_update_secpass_label_by_id(updateData.secpass_id, updateData.label)
