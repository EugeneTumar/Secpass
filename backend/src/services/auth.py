...
from ..db.user import db_get_user_by_signin, db_add_user, db_get_user_by_id, db_update_user
...
async def signIn(signin_info: UserSigninSchema) -> int:
...
async def update_user(session: str, user):
    user_id = await get_user_id_by_session(session)
    await db_update_user(user=User(**crypt_user(user), id=user_id))
